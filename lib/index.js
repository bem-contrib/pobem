...
/**
 * Build RegExp for search bem names
 *
 * @returns {RegExp}
 */
const buildBemRegex = (opts) => {
    ...
    return new RegExp(block + elem + modName + modVal);
}

module.exports = postcss.plugin('pobem', (opts) => {
    const bemRegex = buildBemRegex();
    ...
    return (css) => css.walkRules(rule => {
        ...
        rule.selector = selectorParser(root => {
            root.walk(node => {
                ...
                const entityName = parse(node.value, bemRegex, scope);
